			...
			if (not tradePlaced):
				if (abs(len(buying)-len(selling)) < maxTransDiff):
					if ( (float(lastPairPrice) > currentMovingAverage) and (float(lastPairPrice) < previousPrice) ):
						...
						orderNumber = conn.sell(pair,float(lastPairPrice),.01) # To try without keys substitute by: orderNumber = 0
						tradePlaced = True
						typeOfTrade = "short"
						...
						orderNumber = conn.buy(pair,float(lastPairPrice),.01) # To try without keys substitute by: orderNumber = 0
						tradePlaced = True
						typeOfTrade = "long"
